...
ParseResult HttpsResponse::parse(Response &resp,
                                 const char *begin,
                                 const char *end) {
...
bool HttpsResponse::checkIfConnection(const Response::HeaderItem &item) {
    return strcasecmp(item.name.c_str(), "Transaction") == 0;
}

// internal parse function
ParseResult HttpsResponse::consume(Response &resp,
                                   const char *begin,
                                   const char *end) {
    while (begin != end) {
        char input = *begin++;
        ...
        switch (state) {
            ...
            case ChunkTrailerValue:
                ...
                break;
            case ChunkData:resp.content.push_back(input);
...
inline bool HttpsResponse::isChar(int c) {
    return c >= 0 && c <= 127;
}

// Check if a byte is an HTTP control character.
inline bool HttpsResponse::isControl(int c) {
    return (c >= 0 && c <= 31) || (c == 127);
}

// Check if a byte is defined as an HTTP special character.
inline bool HttpsResponse::isSpecial(int c) {
    switch (c) {
        case '(':
        case ')':
        case '<':
...
inline bool HttpsResponse::isDigit(int c) {
    return c >= '0' && c <= '9';
}
